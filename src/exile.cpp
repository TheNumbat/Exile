...
void exile::init() { PROF
...

	{
		...
		global_api->platform_capture_mouse(&eng->window);
	}
}
...
void exile::update() { PROF
	...
	if(!eng->dbg.show_ui && global_api->platform_window_focused(&eng->window)) {
		global_api->platform_set_cursor_pos(&eng->window, eng->window.w / 2, eng->window.h / 2);
	}
}
