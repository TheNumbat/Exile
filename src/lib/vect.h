...
using vect_base = typename std::conditional<N == 2, v2_base<T>, 
                  typename std::conditional<N == 3, v3_base<T>, 
                  typename std::conditional<N == 4, v4_base<T>, 
...
struct vect : vect_base<T,N> {
    ...
    static constexpr bool is_simd = N == 4 && std::is_same<T,f32>::value;
    ...
    vect normalize() {
        static_assert(std::is_floating_point<T>::value, "Not a float vec!");
        T l = len();
...
T dot(vect<T,N> x, vect<T,N> y) {
    if constexpr(vect<T,N>::is_simd) return v4{_mm_dp_ps(x.pack, y.pack, 0xf1)}.x;
    T r = {};
