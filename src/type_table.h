...
_type_info* get_type_info() {
	return map_get(&type_table, typeid(T).hash_code());
}

void make_type_table(allocator* alloc) {

	type_table = make_map<u64,_type_info>(64, alloc, &hash_u64);

	{
		...
		map_insert(&type_table, hash, u8_t);
	}

	{
		...
		map_insert(&type_table, hash, i8_t);
	}

	{
		...
		map_insert(&type_table, hash, u16_t);
	}

	{
		...
		map_insert(&type_table, hash, i16_t);
	}

	{
		...
		map_insert(&type_table, hash, u32_t);
	}

	{
		...
		map_insert(&type_table, hash, i32_t);
	}

	{
		...
		map_insert(&type_table, hash, u64_t);
	}

	{
		...
		map_insert(&type_table, hash, i64_t);
	}

	{
		...
		map_insert(&type_table, hash, f32_t);
	}

	{
		...
		map_insert(&type_table, hash, f64_t);
	}

	{
		...
		map_insert(&type_table, hash, bool_t);
	}

	{
		...
		map_insert(&type_table, hash, string_t);
	}
